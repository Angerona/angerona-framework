...
import com.github.kreatures.gui.project.ProjectView.UserObjectFactory;
import com.github.kreatures.gui.util.CollectionMonitor.CollectionMonitorListener;
...
/**
 * The presenter wires the KReaturesProject with a view and delegates
 * the view events to the project.
 * 
 * @author Tim Janus
 */
public class ProjectPresenter 
	extends Presenter<KReaturesProject, ProjectView> 
	 ...
	 * @param model	The KReatures project acting as data model
	 * @param view	A class containing Form and Controls which form a view for the KReatures project.
	 */
	public ProjectPresenter(KReaturesProject model, ProjectView view) {
		setModel(model);
		setView(view);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == view.getRemoveButton()) {
			...
			if(selFile != null) {
				...
				} catch (IOException|KReaturesException ex) {
	...
	private class SimUserObject extends ResUserObject<SimulationConfiguration> {
		...
		public void onActivated() {
			KReaturesGUIDataStorage.get().getSimulationControl().setSimulation(getUserObject());
		}
	}
