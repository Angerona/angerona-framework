...
import com.github.kreatures.knowhow.graph.parameter.PlanConverter;
import com.github.kreatures.knowhow.penalty.PenaltyFunction;
import com.github.kreatures.knowhow.penalty.RestrictiveSecretsPenalty;
...
public class KnowhowGraphSubgoal extends SubgoalGenerationOperator {
	...
	private boolean translate(List<WorkingPlan> plans, PlanParameter param, Desire des) {
		...
		for(WorkingPlan plan : plans) {
			...
			LOG.info("Converting WorkingPlan into KReatures format");
					
			// reverse add the intentions to the stack (the list of intentions is ordered in the way that the first 
			// intention has to be done first and the stack of the KReatures planning is ordered the other way round.
