...
/**
 * Base class for all reasoners used in the KReatures framework, sub classes can implement
...
public abstract class BaseReasoner 
	extends Operator<BaseBeliefbase, ReasonerParameter, Pair<Set<FolFormula>, KReaturesAnswer>>{
	...
	@Override
	protected Pair<Set<FolFormula>, KReaturesAnswer> defaultReturnValue() {
		return new Pair<Set<FolFormula>, KReaturesAnswer>(new HashSet<FolFormula>(), null);
	}
	
	@Override 
	protected Pair<Set<FolFormula>, KReaturesAnswer> processImpl(ReasonerParameter params) {
	 ...
	 */
	protected abstract  Pair<Set<FolFormula>, KReaturesAnswer> queryImpl(ReasonerParameter params);
	 ...
	 * @return An KReaturesAnswer which represents the answer to the query
	 */
	public Pair<Set<FolFormula>, KReaturesAnswer> query(BaseBeliefbase caller, FolFormula query) {
		ReasonerParameter params = new ReasonerParameter(caller, query);
		caller.getStack().pushOperator(this);
		Pair<Set<FolFormula>, KReaturesAnswer> reval = processImpl(params);
		caller.getStack().popOperator();
		return reval;
	}
