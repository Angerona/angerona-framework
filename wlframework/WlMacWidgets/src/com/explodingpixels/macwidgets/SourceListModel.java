...
public final class SourceListModel {
    ...
    //ADDED BY MT: top-level items
    //TODO the indices should be checked against the category indices
    private List<SourceListItem> fTopLevelItems = new ArrayList<SourceListItem>();
    ...
    /** 
     * Gets the top-level {@link SourceListItem}s associated with this model. (ADDED BY MT)
     * 
     * @return the top-level {@link SourceListItem}s associated with this model.
     */
    public List<SourceListItem> getTopLevelItems() {
    	return Collections.unmodifiableList(fTopLevelItems);
    }
    
    /**
     * Adds the given item as a top-level item to the model and fires an event such that
     * {@link SourceListModelListener}s will be notified. (ADDED BY MT)
     *
     * @param item the {@link SourceListItem} to add.
     */
    public void addTopLevelItem(SourceListItem item) {
    	addTopLevelItem(item, fTopLevelItems.size());
    }
    
    /**
     * Adds the given item as a top-level item to the model at the given index and fires an event such that
     * {@link SourceListModelListener}s will be notified. (ADDED BY MT)
     *
     * @param item the {@link SourceListItem} to add.
     * @param index the index to add the item at.
     */
    public void addTopLevelItem(SourceListItem item, int index) {
    	fTopLevelItems.add(index, item);
    	item.addPropertyChangeListener(fPropertyChangeListener);
    	fireTopLevelItemAdded(item,index);
    }

    /**
     * Removes the given top-level item from the model and fires an event such that
     * {@link SourceListModelListener}s will be notified. (ADDED BY MT)
     *
     * @param item the {@link SourceListItem} to remove.
     * @throws IllegalArgumentException if the given item is not part of this model.
     */
    public void removeTopLevelItem(SourceListItem item) {
        boolean removed = fTopLevelItems.remove(item);
        if (!removed) {
            throw new IllegalArgumentException("The given top-level item does not exist in this model.");
        }
        fireTopLevelItemRemoved(item);
    }
     ...
     */
    public void removeTopLevelItemAt(int index) {
        SourceListItem item = fTopLevelItems.remove(index);
        if (item == null) {
            throw new IllegalArgumentException("There is no top-level item at the index " + index + ".");
        }
        fireTopLevelItemRemoved(item);
    }
    ...
    public void validateItemIsInModel(SourceListItem item) {
        ...
        // ADDED BY MT
        for (SourceListItem oItem: fTopLevelItems){
        	found = oItem == item || oItem.containsItem(item);
        	if (found) {
                break;
            }
        }
    ...
    //ADDED BY MT
    private void fireTopLevelItemAdded(SourceListItem item, int index) {
        for (SourceListModelListener listener : fListeners) {
            listener.topLevelItemAdded(item, index);
        }
    }

    //ADDED BY MT
    private void fireTopLevelItemRemoved(SourceListItem item) {
        for (SourceListModelListener listener : fListeners) {
            listener.topLevelItemRemoved(item);
        }
    }
