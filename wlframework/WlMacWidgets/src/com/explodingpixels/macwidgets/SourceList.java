...
public class SourceList {
    ...
    //ADDED BY MT
     public void addMouseInputListener(MouseInputListener listener){
    	this.fComponent.addMouseListener(listener);
    	this.fComponent.addMouseMotionListener(listener);
    	this.fTree.addMouseListener(listener);
    	this.fTree.addMouseMotionListener(listener);
    	this.fScrollPane.addMouseListener(listener);
    	this.fScrollPane.addMouseMotionListener(listener);
    }
    
    public void removeMouseInputListener(MouseInputListener listener){
    	this.fComponent.removeMouseListener(listener);
    	this.fComponent.removeMouseMotionListener(listener);
    	this.fTree.removeMouseListener(listener);
    	this.fTree.removeMouseMotionListener(listener);
    	this.fScrollPane.removeMouseListener(listener);
    	this.fScrollPane.removeMouseMotionListener(listener);
    }
    ...
    public SourceList(SourceListModel model) {
        ...
        // add each top-level item and its sub-items to backing JTree. (ADDED BY MT)
        for(int i = 0; i < model.getTopLevelItems().size(); i++) {
        	doAddTopLevelItem(model.getTopLevelItems().get(i), i);
        }        
    }
    ...
    // ADDED BY MT
    private void doAddTopLevelItem(SourceListItem item, int index) {
        DefaultMutableTreeNode itemNode = new DefaultMutableTreeNode(item);
        fTreeModel.insertNodeInto(itemNode, fRoot, index);
        // add each of the item's child items to the tree.
        for (int i = 0; i < item.getChildItems().size(); i++) {
            doAddItemToItem(item.getChildItems().get(i), item, i);
        }
        TreeUtils.expandPathOnEdt(fTree, new TreePath(itemNode.getPath()));
    }
    
    // ADDED BY MT
    private void doRemoveTopLevelItem(SourceListItem item) {
    ...
    //ADDED BY MT
    private void doCheckButtonClick(MouseEvent event) {
    	// ADDED BY MT
    	Object itemOrCategory = getItemOrCategoryUnderPoint(event.getPoint());
    	if(itemOrCategory instanceof ButtonProvider){
    		ButtonProvider btProvider = (ButtonProvider)itemOrCategory;
    		// check whether button has been clicked
    		if(event.getPoint().x >= fTree.getWidth() - btProvider.getPreferredButtonWidth() - 4 && event.getPoint().x <= fTree.getWidth() - 8){
    			btProvider.getButton().doClick();
    			event.consume();
    			return;
    		}
    	}
    ...
    private TreeSelectionListener createTreeSelectionListener() {
        return new TreeSelectionListener() {
            public void valueChanged(TreeSelectionEvent e) {  
            	//BY MT (this is all handled by the mouse listener)
            	//fireSourceListItemSelected(getSelectedItem());
            }
        };
    }
    ...
    private SourceListModelListener createSourceListModelListener() {
        return new SourceListModelListener() {
            ...
            //ADDED BY MT
            public void topLevelItemAdded(SourceListItem item, int index) {
                doAddTopLevelItem(item, index);
            }
            //ADDED BY MT
            public void topLevelItemRemoved(SourceListItem item) {
                doRemoveTopLevelItem(item);
            }
